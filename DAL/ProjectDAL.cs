...
namespace LTTQ_G2_2025.DAL
{
    public class ProjectDAL
    {
        ...
        public bool CreateProjectAndAttachToTeam(
            ...
            bool status, int? semesterId, long teacherId
        )
        {
            string insertSql = @"
                ...
                name, content, img, description, status, (object)semesterId ?? DBNull.Value
                    ...
                    // GÁN CẢ project_id + teacher_id
                    string updateSql = @"UPDATE Team
                                 SET project_id = @pid,
                                     teacher_id = @tch
                                 WHERE team_id = @tid";
                    int n = DataProvider.Instance.ExecuteNonQuery(updateSql, new object[] {
                newProjectId, teacherId, teamId
            });
            return n > 0;
        }
